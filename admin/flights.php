				...
				<table class="table table-bordered" id="flight-list">
					...
					<thead>
						<tr>
							...
							<th class="text-center">Adult Price</th>
					...
					<tbody>
						<?php
							...
							$qry = $conn->query("SELECT f.*,a.airlines,a.logo_path FROM flight_details f inner join airlines_list a on f.airline_id = a.airline_id  order by flight_id desc");
						 ...
						 <tr>
						 	...
						 	<td class="text-right"><?php echo number_format($row['adult_price'],2) ?></td>
							 <td class="text-right"><?php echo number_format($row['child_price'],2) ?></td>
...
function delete_flight($id){
		...
		$.ajax({
			...
			success:function(resp){
				...
				else {alert_toast("Can not be deleted",'fail')
					setTimeout(function(){
						location.reload()
					},1500)}
			}
